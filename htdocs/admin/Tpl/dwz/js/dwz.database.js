	...
	$.fn.extend({
		...
		lookupselect: function(){
            return this.each(function(){
                ...
                $this.click(function(event){
                    _lookup = $.extend(_lookup, {
                        currentGroup: $this.attr("lookupGroupSelect") || "",
                        suffix: $this.attr("suffix") || "",
                        $target: $this,
                        pk: $this.attr("lookupPk") || "id"
                    });
                    ...
                    var groupname = $this.attr("lookupGroupSelect");
                    options.data = {};
                    $.each($(navTab.getCurrentPanel()).find("input[lookupSend='"+groupname+"_data']"),function(i,n){
                        var key = "selected_"+$(n).attr("name").replace(groupname+".","");
                        options.data[key] = $(n).val();
                    });
                     $.pdialog.open(url, "_blank", $this.attr("title") || $this.text(), options);
                    return false;
                });
		...
		//增加by hc 用于可保存分页选择的回调
		multLookupSelected: function(){
            return this.each(function(){
                var $this = $(this), args={};
                $this.click(function(event){            
                    $.bringBack(selected_data);
                });
            });
        },
