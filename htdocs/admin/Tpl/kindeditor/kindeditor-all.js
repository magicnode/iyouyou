...
function _drag(options) {
	...
	if (iframeFix) {
		K('iframe').each(function() {
			var src = _formatUrl(this.src || '', 'domain');
			if (/^https?:\/\//.test(src)) {
				return;
			}
			var doc;
...
function _loadStyle(url) {
	var head = document.getElementsByTagName('head')[0] || (_QUIRKS ? document.body : document.documentElement),
		...
		absoluteUrl = _chopQuery(_formatUrl(url, 'domain'));
	var links = K('link[rel="stylesheet"]', head);
	for (var i = 0, len = links.length; i < len; i++) {
		if (_chopQuery(_formatUrl(links[i].href, 'domain')) === absoluteUrl) {
			return;
		}
	}
...
KindEditor.plugin('baidumap', function(K) {
	...
	self.clickToolbar(name, function() {
		var html = ['<div style="padding:10px 20px;">',
			...
			'<input type="hidden" id="kindeditor_plugin_map_xpoint" />',
			'<input type="hidden" id="kindeditor_plugin_map_ypoint" />',
			'</div>'].join('');
		var dialog = self.createDialog({
			name : name,
			width : mapWidth + 42,
...
KindEditor.plugin('filemanager', function(K) {
	...
	self.plugin.filemanagerDialog = function(options) {
		...
		function bindEvent(el, result, data, createFunc) {
			var fileUrl = K.formatUrl(result.current_url + data.filename, 'domain'), //hc 浏览图片时
...
KindEditor.plugin('image', function(K) {
	...
	self.plugin.imageDialog = function(options) {
		...
		var html = [
			...
			IS_WATER==1?'<label style="width:60px;"><select name="is_water"><option value="1">有水印</option><option value="0">无水印</option></select></label>':"",		
			'<input type="text" name="localUrl" class="ke-input-text" tabindex="-1" style="width:150px;" readonly="true" /> &nbsp;',
			'<input type="button" class="ke-upload-button" value="' + lang.upload + '" />',
			'</div>',
			'</form>',
			'</div>',
		...
		var uploadbutton = K.uploadbutton({
			...
			afterUpload : function(data) {
				...
				if (data.error === 0) {
					var url = data.url;
					if (formatUploadUrl) {
						url = K.formatUrl(url, 'domain'); //hc 图片控件以domain urlType方式上传
					}
					if (self.afterUpload) {
						self.afterUpload.call(self, url, data, name);
					}
...
K.extend(KSWFUpload, {
	init : function(options) {
		...
		self.div = K(options.container).html([
			...
			IS_WATER==1?'<select name="is_water" class="multi_is_water"><option value=1>有水印</option><option value=0>无水印</option></select>':'',
		...
		K('.ke-swfupload-startupload input', self.div).click(function() {
		    self.swfu.setUploadURL(options.uploadUrl+"&is_water="+$(self.div).find("select.multi_is_water").val());
