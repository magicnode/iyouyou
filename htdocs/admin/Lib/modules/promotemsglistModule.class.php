...
class promotemsglistModule extends AuthModule
{
	...
	{		
		...
		$list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."promote_msg_list where ".$condition." order by ".$param['orderField']." ".$param["orderDirection"]." limit ".$limit);
		$totalCount = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."promote_msg_list where ".$condition);
		...
		$GLOBALS['tmpl']->assign("formaction",admin_url("promotemsglist"));
		$GLOBALS['tmpl']->assign("delurl",admin_url("promotemsglist#foreverdelete",array('ajax'=>1)));
		$GLOBALS['tmpl']->assign("viewurl",admin_url("promotemsglist#show_content",array('ajax'=>1)));
		$GLOBALS['tmpl']->assign("sendurl",admin_url("promotemsglist#send",array('ajax'=>1)));
		$GLOBALS['tmpl']->assign("reseturl",admin_url("index#reset_sending",array('field'=>"PROMOTE_MSG_LOCK","ajax"=>1)));
		$GLOBALS['tmpl']->display("core/promotemsglist/index.html");
	}	
	...
	public function foreverdelete()
	 {
		...
		if (isset ( $_REQUEST ['id'] ))
		{
			...
			if($id)
			{
				$sql = "delete from ".DB_PREFIX."promote_msg_list where id in (".$id.")";
	...
	public function send()
	{
		...
		if($msg_item)
		{
			if($msg_item['send_type']==0)
			{
				...
				$GLOBALS['db']->autoExecute(DB_PREFIX."promote_msg_list",$msg_item,"UPDATE","id=".$id);
				if($result['status'])
				{
	...
	{
		$id = intval($_REQUEST['id']);
		$content = $GLOBALS['db']->getOne("select content from ".DB_PREFIX."promote_msg_list where id = ".$id);
		$content = htmlspecialchars($content);
		$GLOBALS['tmpl']->assign("content",$content);
		$GLOBALS['tmpl']->display("core/promote_msg_list/show_content.html");
	}
