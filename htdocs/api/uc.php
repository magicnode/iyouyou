...
class uc_note {
        ...
        if(!$user_info)
        {
        	...
        	if($uc_data = uc_get_user($username))
			{
		       	...
		       	if(!$GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user where user_name = '".$uname."'")>0) 
				{
					...
					$user_info['regist_id'] = CLIENT_IP;
					$user_info['create_time'] = NOW_TIME;

					require_once APP_ROOT_PATH."system/libs/city.php";
					$user_info['regist_city'] = City::locate_city_name(CLIENT_IP);
    ...
    function updatepw($get, $post)
    {
        ...
        {
        	$salt = USER_SALT;
        	$newpw = md5($get['password'].USER_SALT);
        	$GLOBALS['db']->query("UPDATE " .DB_PREFIX."user SET user_pwd='$newpw',salt='".$salt."' WHERE user_name='$username'");
        }       
