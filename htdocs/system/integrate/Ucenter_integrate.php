...
class Ucenter_integrate implements integrate {
	...
	{		
		...
		if($uid>0)
		{
				...
				{
					//被同步过的存在于ucenter中的用户
					$GLOBALS['db']->query("update ".DB_PREFIX."user set user_pwd = '".md5($user_pwd.USER_SALT)."',salt='".USER_SALT."' where id = ".$ease_user['id']);
				}	
				elseif(!$ease_user)
				{
					if(!$GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user where user_name = '".$uname."'")>0) 
					{
						...
						$ease_user['is_verify'] = 1;
						if($email!="")
						$ease_user['email'] = $email;
						$ease_user['user_name'] = $ease_user['nickname'] = $uname;
						$ease_user['user_pwd'] = md5($user_pwd.USER_SALT);
						$ease_user['salt'] = USER_SALT;
						$ease_user['integrate_id'] = $uid;
						$ease_user['regist_ip'] = CLIENT_IP;
						
						require_once APP_ROOT_PATH."system/libs/city.php";
						$ease_user['regist_city'] = City::locate_city_name(CLIENT_IP);
		...
		return array('status'=>0,'data'=>0); //无会员返回0
	}
	...
	public function add_user($user_name,$user_pwd,$email)
	{
		...
		if ($email!=""&&$this->check_email($email)) //如存在
        {
			$res['field'] = 'email';
			$res['info']	=	"邮箱已存在";
			$res['status'] = 0;
