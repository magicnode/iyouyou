...
class AppTemplate
{
    ...
    {    	
	        ...
	        {        
	        	$code = load_dynamic_cache("adv_id_".$adv_id.$GLOBALS['city']['py']);
	        	if($code===false)
	        	{
	        		$code =  load_auto_cache("adv_cache",array("adv_id"=>$adv_id,"city_py"=>$GLOBALS['city']['py']));
	        		set_dynamic_cache("adv_id_".$adv_id.$GLOBALS['city']['py'], $code);
	        	}
	        	$out = preg_replace("/<adv(\s+)adv_id=\"".$adv_id."\"([^>]*)>/i",$code,$out,1);
    				...
    				if(count($matches[0])>0)
    				{
    					...
    					if(isset($matches[0][1])&&$matches[0][1]!='')
    					{
    						...
    						for($kk=1;$kk<count($matches[0]);$kk++)
    						{
    							...
    							if(count($param_matches[0])>0)
    							$param.=  ',$this->_var["'.$param_matches[1][0].'"]';
    							else
    							$param.= ",".$matches[0][$kk];
    						}    						
    					}
    ...
    function insert_mod($name) // 处理动态内容
    {
        ...
        require_once APP_ROOT_PATH.APP_NAME."/Lib/insert_libs.php";	
        if(!function_exists($insert_func))//也可调用module中的静态方法
        {
       		require_once APP_ROOT_PATH.APP_NAME."/Lib/inserts/".strtolower(MODULE_NAME)."_insert_libs.php";	
        }
