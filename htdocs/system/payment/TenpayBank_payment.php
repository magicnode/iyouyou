...
class TenpayBank_payment implements payment {
	...
	public function get_payment_code($user_data,$order_type,$order_sn,$subject,$money)
	{
		...
		$GLOBALS['db']->query("update ".DB_PREFIX."payment_notice set payment_class='TenpayBank',payment_name='财付通银行直连支付' where notice_sn = '".$payment_notice_sn."'");
		$payment_notice = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."payment_notice where notice_sn = '".$payment_notice_sn."'");
		
		$money = format_price_to_display($payment_notice['money']);
		$payment_info = $GLOBALS['db']->getRow("select id,config,logo from ".DB_PREFIX."payment where class_name = 'TenpayBank' ");
		if(empty($payment_info))app_redirect(url("index"));
		$payment_info['config'] = unserialize($payment_info['config']);

		
		$data_return_url = url("transaction#response",array("class_name"=>"TenpayBank"));
		$data_notify_url = url("transaction#notify",array("class_name"=>"TenpayBank"));
		...
		$payLinks .= "<input type='submit' class='paybutton' style='display:none;' />正在连接支付网关...</form>";
	...
	public function response($request)
	{
    	...
    	{		
    		...
    		$pay_result = Transaction::pay_payment($payment_notice, $outer_notice_sn,"TenpayBank");
	...
	{		
		...
		if($payment_item)
		{
			...
			$payment_item['bank_list'] = $payment_cfg['tencentpay_gateway'];

			foreach ($payment_item['bank_list'] AS $key=>$val)
	        {
	        	$bank_item = array();
	        	$bank_item['logo'] = SITE_DOMAIN.APP_ROOT."/system/payment/TenpayBank/".$val.".gif";
	        	$bank_item['name'] = $this->payment_lang['tencentpay_gateway_'.$val];
