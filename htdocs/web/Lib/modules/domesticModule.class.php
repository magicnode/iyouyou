...
class domesticModule extends BaseModule{
    ...
    function index() {
		...
		if (!$GLOBALS['tmpl']->is_cached('tourline_domestic_index.html', $cache_id))
		{	
			//输出推荐线路
			$recommend_tourline_layer = array();
			require_once APP_ROOT_PATH."system/libs/tourline.php";
				
			$recommend_tourlist = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."tourline_rec_config where rec_page = 1 order by rec_sort asc");
			
			foreach($recommend_tourlist as $k=>$v)
			{
				$rec_result = load_tourline_rec(1, $v['rec_type'], $v['rec_id']);
    	...
    	$GLOBALS['tmpl']->display("tourline_domestic_index.html",$cache_id);
    }
