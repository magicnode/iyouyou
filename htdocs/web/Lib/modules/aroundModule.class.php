...
class aroundModule extends BaseModule{
    ...
    function index() {
		...
		if (!$GLOBALS['tmpl']->is_cached('tourline_around_index.html', $cache_id))
		{	
			//输出推荐线路
			$recommend_tourline_layer = array();
			require_once APP_ROOT_PATH."system/libs/tourline.php";
			
			$recommend_tourlist = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."tourline_rec_config where rec_page = 3 order by rec_sort asc");
				
			foreach($recommend_tourlist as $k=>$v)
			{
				$rec_result = load_tourline_rec(3, $v['rec_type'], $v['rec_id']);
			...
			init_app_page();
			//输出SEO元素
			$GLOBALS['tmpl']->assign("site_name","周边游 - ".app_conf("SITE_NAME"));
			$GLOBALS['tmpl']->assign("site_keyword","周边游,".app_conf("SITE_KEYWORD"));
			$GLOBALS['tmpl']->assign("site_description","周边游,".app_conf("SITE_DESCRIPTION"));
		}
    	$GLOBALS['tmpl']->display("tourline_around_index.html",$cache_id);
    }
