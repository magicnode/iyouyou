    ...
    function init() {
        ...
        */
        ...
        var product = function (opt) {
            ...
            this.traintmp = '<div class="show_list_box show_flights_box show_train_box" style="display:;">\
						<div class="show_list show_flights">\
							<ul>\
								{{each searchResult.train}}\
								<li>\
							        <div class="li_box">\
								        {{if config.train.fdDate==true}}<p class="time">${date}</p>{{/if}}\
								        {{if config.train.fdStation==true}}<p class="line">${deptCity} - ${arriveCity}</p>{{/if}}\
								        <p class="type">{{if config.train.isTrainNo==true}}${trainNo}{{/if}}&nbsp;&nbsp;&nbsp;&nbsp;{{if config.train.useTime==true}}${useTimeStr}{{/if}}</p>\
								        <p class="price">{{if config.train.price==true}}<dfn>￥</dfn><em>${price}</em>{{/if}} <a href="${bookUri}" target="_blank" class="book">预订</a></p>\
							        </div>\
                                </li>\
								{{/each}}\
							</ul>\
...
'<style type="text/css">a {color:${config.diyTheme.linkText};} </style> '];
		...
		<ul>{{each config.searchOption.holiday}}<li class="${classs}"><div class="search_box ${classss}"><span class="hd">${name}</span>${value}</div></li>{{/each}}<li><div class="search_box"><button type="button" id="holidaysubmit">搜索</button>{{if config.holiday.isYoupiao==true}}<a href="http://u.ctrip.com/union/CtripRedirect.aspx?TypeID=2&Allianceid=${config.AllianceId}&sid=${config.SId}&OUID=${config.OuId}&jumpUrl=http://lipin.ctrip.com/sale/ypbooking/index.aspx" target="_blank" style="margin-left:13px;">礼品卡</a>{{/if}}</div></li></ul></div>{{/if}}<!-- 机票搜索 结束 -->';
		...
		{{if config.searchOption.group}}{{each config.searchOption.group}}<li class="${classs}"><div class="search_box ${classss}"><span class="hd">${name}</span>${value}</div></li>{{/each}}{{/if}}\
		<li><div class="search_box"><button type="button" id="groupSearchBt">搜索</button>{{if config.group.isYoupiao==true}}<a href="http://u.ctrip.com/union/CtripRedirect.aspx?TypeID=2&Allianceid=${config.AllianceId}&sid=${config.SId}&OUID=${config.OuId}&jumpUrl=http://lipin.ctrip.com/sale/ypbooking/index.aspx" target="_blank" style="margin-left:13px;">礼品卡</a>{{/if}}</div></li></ul></div> {{/if}}';
                this.tmpls += '<!--团购搜索 结束-->'

                var _trainHeadTmp = '{{if config.siteTypeValue=="train" || config.siteMode=="all"}}\
                        <div style="{{if config.siteTypeValue!="train"}}display:none;{{/if}}" id="trainOption" class="search_list activites_search_list" style="*position:relative">\
                            <ul>\
                            {{if config.searchOption.train}}\
                            {{each config.searchOption.train}}\
                                <li class="${classs}"><div class="search_box ${classss}"><span class="hd">${name}</span>${value}</div></li>\
                            {{/each}}\
                            {{/if}}\
                            <li><div class="search_box"><button type="button" id="trainSearchBt">搜索</button>{{if config.train.isYoupiao==true}}<a href="http://u.ctrip.com/union/CtripRedirect.aspx?TypeID=2&Allianceid=${config.AllianceId}&sid=${config.SId}&OUID=${config.OuId}&jumpUrl=http://lipin.ctrip.com/sale/ypbooking/index.aspx" target="_blank" style="margin-left:13px;">礼品卡</a>{{/if}}</div></li>\
                            </ul>\
                        </div>\
                       {{/if}}';

                //火车票 开始
                this.tmpls += _trainHeadTmp;
                //火车票 结束

                this.tmpls += '</div><!-- 搜索列表 结束 -->';
                ...
                this.tmpls += '<div class="c_sort_select" id="selSort" >\
                    {{if config.siteTypeValue=="hotel"}}\
                    <a href="javascript:void(0);"><b></b><span>默认排序：携程推荐</span></a>\
                    <div class="c_sort_list" id="sort_select" style="width:142px;display:none;">\
                     ...
                     {{else}}\
                     <a href="javascript:void(0);"><b></b><span>默认排序：携程推荐</span></a>\
                    <div style="width:142px; display: none;" id="sort_select" class="c_sort_list">\
						<a href="javascript:void(0);" name="startDate|asc">发车时间由早到晚</a>\
						<a href="javascript:void(0);" name="startDate|desc">发车时间由晚到早</a>\
						<div class="value_nono"></div>\
						<a href="javascript:void(0);" name="useTime|asc">耗时由短到长</a>\
						<a href="javascript:void(0);" name="useTime|desc">耗时由长到短</a>\
						<div class="value_nono"></div>\
						<a href="javascript:void(0);" name="default">默认排序：携程推荐</a>\
					</div>\
                     {{/if}}\
                 </div>'
                this.tmpls += '</div>';
            ...
            data: {
                config: {
                    ...
                    col: 2,
                    row: 5,
                    trainDefaultDeptCity: null,
                    trainDefaultArriveCity: null,
                    trainDefaultDeptDate: null,
                    hidtrainDDCity: null,
                    hidtrainDACity: null,
            ...
            abback: {
                st: 'siteTypeValue', sm: 'siteMode', smi: 'siteModeItems', hp: 'hotel', tp: 'ticket', hop: 'holiday', gr: 'group', si: 'skinId', ti: 'themeId', had: 'hostAddress', num: 'skinNum',
                id: 'isDiytheme', sl: 'showLogo', sd: 'showData', hsp: 'hotelOption', tsp: 'ticketOption', hosp: 'holidayOption', grp: 'groupOption', lt: 'linkText', hd: 'header', sa: 'searchArea',
                pt: 'priceText', hc: 'headBackColor', dc: 'dataBackColor', fc: 'footBackColor', pc: 'pageBackColor', da: 'dataArea', bd: 'border', ht: 'headText', diy: 'diyTheme', opt: 'opt', pw: 'pageWidth',
                cl: 'col', rw: 'row', gri: 'groupDefaultCityID', grd: 'groupDefaultCity', gpy: 'groupDefaultCityPY', hdc: 'hotelDefaultCity', tdb: 'ticketDefaultBegin', hfc: 'holidayDefaultCity',
                fv: 'fixedValue', ifs: 'isFixed', so: 'searchOption', hac: 'holidayAimCity', tde: 'ticketDefaultEnd', ph: 'pageHeight', ft: 'footHeight', allid: 'AllianceId', sid: 'SId', oid: 'OuId',
                'app': 'App', hdt: 'hotelInTime', hat: 'hotelOutTime', tdt: 'flightDepTime', fat: 'flightArriveTime', hhd: 'hotelhidD', hod: 'holidayHidD', hoa: 'holidayHidA', thd: 'ticketHidD', tha: 'thicketHidA',
                tn: 'train', ddc: 'trainDefaultDeptCity', dac: 'trainDefaultArriveCity', tdd: 'trainDeptDate', tbd: 'trainBackDate', trsp: 'trainOption', hddc: 'hidtrainDDCity', hdac: 'hidtrainDACity', tddd: 'trainDefaultDeptDate'
            },
            analypara: function (para) {
                ...
                //modify by s27110 on 20130131
                if (config["AllianceId"]) {
                    this.data.config.AllianceId = config["AllianceId"];
                };
                if (config["SId"]) {
                    this.data.config.SId = config["SId"];
                };
                if (config["OuId"]) {
                    this.data.config.OuId = config["OuId"];
                }
                ...
                if (this.data.config.siteMode == 'simple') {
                    ...
                    } else if (this.data.config.siteTypeValue == 'train') {
                        this.data.config.trainDefaultDeptDate = config['trainDefaultDeptDate'];
                    }
                } else {
                    ...
                    if (config['group']) setTrue(this.data.config['group'], config['group'], '|');
                    if (config['train']) setTrue(this.data.config['train'], config['train'], '|');
                    this.data.config.hotelInTime = config['hotelInTime'];
                    this.data.config.hotelOutTime = config['hotelOutTime'];
                    this.data.config.flightDepTime = config['flightDepTime'];
                    this.data.config.flightArriveTime = config['flightArriveTime'];
                ...
                if (config['trainDefaultDeptCity']) {
                    this.data.config['trainDefaultDeptCity'] = config['trainDefaultDeptCity']
                }
                if (config['trainDefaultArriveCity']) {
                    this.data.config['trainDefaultArriveCity'] = config['trainDefaultArriveCity'];
                }
                if (config['hidtrainDDCity']) {
                    this.data.config['hidtrainDDCity'] = config['hidtrainDDCity'];
                }
                if (config['hidtrainDACity']) {
                    this.data.config['hidtrainDACity'] = config['hidtrainDACity'];
                }
                if (this.data.config['siteMode'] == 'simple') {
                            ...
                            for (var i = 0, l = config['siteModeItems'].split('|').length; i < l; i++) {
                                switch (config['siteModeItems'].split('|')[i]) {
                                    ...
                                    case "g":
                                        ...
                                        break;
                                    case "t":
                                        html.push('<a href="javascript:void(0)" name="train" class=" last_child"><span><b class="ico ico_train"></b>火车票</span></a>');
                                        break;
                                }
                            }
                ...
                if (this.data.config['siteMode'] == 'simple') {
                    ...
                    if (config['groupOption']) this.data.config.searchOption['group'] = setOptionValue(this.defineData.searchOption['group'], 'group');
                    if (config['trainOption']) this.data.config.searchOption['train'] = setOptionValue(this.defineData.searchOption['train'], 'train');
                }
                //颜色数据
                ...
                if (this.data.config.siteTypeValue == 'hotel') {
                    ...
                    this.App = '0104B00';
                } else if (this.data.config.siteTypeValue == 'holiday') {
                    this.App = '0104C00';
                } else if (this.data.config.siteTypeValue == 'group') {
                    this.App = '0104D00';
                } else if (this.data.config.siteTypeValue == 'train') {
                    this.App = '0104G00';
                }
                    ...
                    this.hostAddress = (function () {
                        ...
                        switch (config['hostAddress']) {
                            ...
                            case 'dev':
                                ...
                                //南通服务器
                            case 'fat1':
                                return 'http://u.fat1.qa.nt.ctripcorp.com';
                            default:
                                return 'http://u.ctrip.com/';
                        }
            ...
            updateView: function (opt) {
                ...
                if (pa == null) {
                    //var sc=$("script");
                    //这个id是第三方引用时才有的
                    var scr = cQuery("#ctrip_union_product_20121221")[0];
                    pa = scr.src.split('?')[1];
                }
            ...
            writedom: function (appendPosi) {
                ...
                if (!appendPosi || appendPosi == null) {
                    //var sc=cQuery("script");
                    var scr = cQuery("#ctrip_union_product_20121221")[0];
                    scr.parentNode.appendChild(x);
                }
                else {
                    cQuery("#" + appendPosi)[0].innerHTML = '';
                    cQuery("#" + appendPosi)[0].appendChild(x);
                }

                this.ifm = x;
                var w = x.contentWindow;

                var showvalue = cQuery.tmpl.render(this.tmpls, this.datas);
                w.document.open();
               ...
               //var styleCalendar = document.createElement("style");
                ...
                try {
                    //styleCalendar.appendChild(document.createTextNode("div.calendar_wrap .calendar_content .calendar_day dd{margin:0;}div.calendar_wrap .calendar_content .calendar_day{margin-top:0px;margin-bottom:0;}"));
                styleCalendar.appendChild(document.createTextNode("#calendars #calendar1{}"))
                } catch (ex) {
                    styleCalendar.styleSheet.cssText = "div.calendar_wrap .calendar_content .calendar_day dd{margin:0;}div.calendar_wrap .calendar_content .calendar_day{margin-top:0px;margin-bottom:0;}";
                }         

                window.document.getElementsByTagName('head')[0].appendChild(styleCalendar)            */
                if (window["b2b_ctrip_v2"] && window["b2b_ctrip_current_content_height"]) {
                    // cQuery("#undefined")[0].contentWindow.document.getElementById('b2b_content').style.height = window["b2b_ctrip_current_content_height"] + 'px';
                    //cQuery("#undefined")[0].contentWindow.document.getElementById('_main_box').style.height =this.data.config.pageHeight-38 + 'px';
                };
                this.bindEvent();
                return true;
            },
            bindEvent: function () {
                ...
                if (changeSiteList.length > 0) {
                    ...
                    changeSiteList.bind("click", function (e) {
                        ...
                        for (var i = 0; i < changeSiteList.length; i++) {
                            ...
                            cQuery("#b2b_content", doc)[0].style.visibility = 'hidden'
                            cQuery("#b2b_page", doc)[0].style.visibility = 'hidden'
                        }
                        if (cQuery(this)[0].className == "last_child") {
                            cQuery(this)[0].className = "current last_child";
                        } else {
                            cQuery(this)[0].className = "current";
                        }
                        ...
                        var groupArea = self.ifm.contentWindow.document.getElementById("groupOption");
                        var trainArea = self.ifm.contentWindow.document.getElementById("trainOption");
                        ...
                        self.datas.config.headerHeight = parseInt(cQuery("#show_search_box", doc).offset()['height']);
                        content.dataType = types
                        //content.accountPageInfo(self.datas.config);
                        if (self.siteCallback) {
                            self.siteCallback(types);
                        }
                        if (cQuery("#ctrip_union_product_20121221")[0] || !cQuery("#ctrip_union_product_20121221")[0]) {
                            if (cQuery("#showDataList")[0]) { self.data.config.showData = cQuery("#showDataList")[0].checked }
                            // var searchBoxHeight = parseInt(cQuery("#b2b_searchbox", doc).offset()['height']);
                            ...
                            if ((self.data.config.showData && self.data.config.pageHeight - searchBoxHeight - bootBoxHeight > content.minContentHeight) && (self.data.config.pageWidth >= 270)) {
                                ...
                                cQuery("#b2b_searchOption", doc)[0].style.height = "auto"
                                switch (types) {
                                    case 'hotel': cQuery.event.trigger(cQuery('#hotelsubmit', doc), 'click');
                                        break;
                                    case 'holiday': cQuery.event.trigger(cQuery('#holidaysubmit', doc), 'click');
                                        break;
                                    case 'ticket': cQuery.event.trigger(cQuery('#ticketSearchBt', doc), 'click');
                                        break;
                                    case 'group': cQuery.event.trigger(cQuery('#groupSearchBt', doc), 'click');
                                        break;
                                    case 'train': cQuery.event.trigger(cQuery('#trainSearchBt', doc), 'click');
                                        break;
                                };
                                cQuery("#b2b_searchOption", doc)[0].style.height = "auto";
                            } else {
                                cQuery("#b2b_searchOption", doc)[0].style.height = content.getMainboxHeight(doc, self.data.config, "title") + "px";
                            }
                        }
                    });
                }
                if (cQuery("#flightway", doc).length > 0) {
                    cQuery("#flightway", doc).bind("change", function (e) {
                        var types = cQuery("#flightway", doc)[0].value;
                        if (cQuery("#ADatePeriod1", doc).length > 0 && types == "Simple") {
                            cQuery("#ADatePeriod1", doc)[0].parentNode.parentNode.parentNode.style.display = "none";
                        } else if (cQuery("#ADatePeriod1", doc).length > 0 && types == "Double") {
                            cQuery("#ADatePeriod1", doc)[0].parentNode.parentNode.parentNode.style.display = "";
                        }
                    });
                }
                var trainTypeRadios = cQuery('*[name=train_type]', doc);
                if (trainTypeRadios.length == 2) {
                    trainTypeRadios.bind('click', function () {
                        var showDataC;
                        if (trainTypeRadios[0].checked) cQuery(cQuery('#p_tAdate', doc)[0].parentNode.parentNode.parentNode).addClass('labgray');
                        else cQuery(cQuery('#p_tAdate', doc)[0].parentNode.parentNode.parentNode).removeClass('labgray');
                        if (cQuery("#showDataList")[0]) { showDataC = union.data.config.showData; } else { showDataC = content.ctripUnion.datas.config.showData; };
                        if (showDataC) triggerClick('train', doc);
                    });
                };
                //设置默认城市
                if (this.datas.config.hotelDefaultCity && cQuery("#cityname", doc).length > 0) {
                    cQuery("#cityname", doc)[0].value = this.datas.config.hotelDefaultCity;
                    cQuery("#citynameInter", doc)[0].value = this.datas.config.hotelDefaultInterCity;
                    cQuery("#cityId", doc)[0].value = this.datas.config.hotelhidD;
                    cQuery("#DistrictId", doc)[0].value = this.datas.config.hotelInertHid;
                }
                if (this.datas.config.ticketDefaultBegin && cQuery("#homecity_name", doc).length > 0) {
                    cQuery("#homecity_name", doc)[0].value = this.datas.config.ticketDefaultBegin;
                    cQuery("#homecity_nameOut", doc)[0].value = this.datas.config.ticketDefaultBeginOut;
                    cQuery("#homecity", doc)[0].value = this.datas.config.ticketHidD;
                    cQuery("#homecityOut", doc)[0].value = this.datas.config.ticketHidDOut;
                }
                if (this.datas.config.ticketDefaultEnd && cQuery("#destcity1_name", doc).length > 0) {
                    cQuery("#destcity1_name", doc)[0].value = this.datas.config.ticketDefaultEnd;
                    cQuery("#destcity1_nameOut", doc)[0].value = this.datas.config.ticketDefaultEndOut;
                    cQuery("#destcity1", doc)[0].value = this.datas.config.thicketHidA;
                    cQuery("#destcity1Out", doc)[0].value = this.datas.config.thicketHidAOut;
                }
                if (this.datas.config.holidayDefaultCity && cQuery("#text1", doc).length > 0) {
                    cQuery("#text1", doc)[0].value = this.datas.config.holidayDefaultCity;
                    cQuery("#text1ID", doc)[0].value = this.datas.config.holidayHidD;
                }
                if (this.datas.config.holidayAimCity && cQuery("#pkgdestCity", doc).length > 0) {
                    cQuery("#pkgdestCity", doc)[0].value = this.datas.config.holidayAimCity;
                    cQuery("#pkgdestCityID", doc)[0].value = this.datas.config.holidayHidA;
                }
                if (cQuery("#groupCity", doc).length > 0) {
                    if (this.datas.config.groupDefaultCity)
                        cQuery("#groupCity", doc)[0].value = this.datas.config.groupDefaultCity;
                    else
                        cQuery("#groupCity", doc)[0].value = "";
                }
                if (cQuery("#grouphidCity", doc).length > 0) {
                    if (this.datas.config.groupDefaultCityID)
                        cQuery("#grouphidCity", doc)[0].value = this.datas.config.groupDefaultCityID;
                    else
                        cQuery("#grouphidCity", doc)[0].value = "";
                }
                if (cQuery("#grouphidCityPinYin", doc).length > 0) {
                    if (this.datas.config.groupDefaultCityPY)
                        cQuery("#grouphidCityPinYin", doc)[0].value = this.datas.config.groupDefaultCityPY;
                    else
                        cQuery("#grouphidCityPinYin", doc)[0].value = "";
                };
                if (cQuery('#p_trainDcity', doc).length > 0) {
                    cQuery('#p_trainDcity', doc).value(this.datas.config.trainDefaultDeptCity || '');
                };
                if (cQuery('#p_trainAcity', doc).length > 0) {
                    cQuery('#p_trainAcity', doc).value(this.datas.config.trainDefaultArriveCity || '');
                };
                if (cQuery('#phid_tDcity', doc).length > 0) {
                    cQuery('#phid_tDcity', doc).value(this.datas.config.hidtrainDDCity || '');
                };
                if (cQuery('#phid_tAcity', doc).length > 0) {
                    cQuery('#phid_tAcity', doc).value(this.datas.config.hidtrainDACity || '');
                };
                //设置默认时间
                if (this.datas.config.hotelInTime) {
                    ...
                    if (cQuery("#starttime", doc).length > 0)
                        cQuery("#starttime", doc)[0].value = this.datas.config.hotelInTime;
                    else if (cQuery("#hotelOption", doc).length > 0) {
                        var tmp = doc.createElement("input");
                        tmp.id = "starttime";
                        tmp.type = "hidden";
                        tmp.value = this.datas.config.hotelInTime;
                        cQuery("#hotelOption", doc).append(tmp);
                    }
                }
                if (this.datas.config.hotelOutTime) {
                    ...
                    if (cQuery("#deptime", doc).length > 0)
                        cQuery("#deptime", doc)[0].value = this.datas.config.hotelOutTime;
                    else if (cQuery("#hotelOption", doc).length > 0) {
                        var tmp = doc.createElement("input");
                        tmp.id = "deptime";
                        tmp.type = "hidden";
                ...
                if (this.datas.config.flightDepTime) {
                    ...
                    //modify by s27110 on 2013-01-06
                    var _today = now.getFullYear() + '-' + month + '-' + days;
                    if (doc.forms["flightForm"]) doc.forms["flightForm"]["today"].value = _today;

                    if (cQuery("#DDatePeriod1", doc).length > 0)
                        cQuery("#DDatePeriod1", doc)[0].value = this.datas.config.flightDepTime;
                    else if (cQuery("#ticketOption", doc).length > 0) {
                        var tmp = doc.createElement("input");
                        tmp.id = "DDatePeriod1";
                        tmp.type = "hidden";
                        tmp.value = this.datas.config.flightDepTime;
                        cQuery("#ticketOption", doc).append(tmp);
                    }
                }
                if (this.datas.config.flightArriveTime) {
                    ...
                    if (cQuery("#ADatePeriod1", doc).length > 0)
                        cQuery("#ADatePeriod1", doc)[0].value = this.datas.config.flightArriveTime;
                    else if (cQuery("#ticketOption", doc).length > 0) {
                        var tmp = doc.createElement("input");
                        tmp.id = "ADatePeriod1";
                        tmp.type = "hidden";
                ...
                if (this.datas.config.trainDefaultDeptDate) {
                    var today = new Date();
                    var deptDate = new Date(today.getFullYear(), (today.getMonth()), (today.getDate() + Number(this.datas.config.trainDefaultDeptDate)));
                    var arriDate = new Date(today.getFullYear(), (today.getMonth()), (today.getDate() + Number(this.datas.config.trainDefaultDeptDate) + 1));
                    var deptStr = deptDate.getFullYear() + '-' + (deptDate.getMonth() + 1) + '-' + deptDate.getDate();
                    var arriStr = arriDate.getFullYear() + '-' + (arriDate.getMonth() + 1) + '-' + arriDate.getDate();
                    if (cQuery('#p_tDdate', doc).length > 0) {
                        cQuery('#p_tDdate', doc).value(deptStr);
                    };
                    if (cQuery('#p_tAdate', doc).length > 0) {
                        cQuery('#p_tAdate', doc).value(arriStr);
                    };
                };
            },
            defineData: {
                ...
                searchOption: { hotel: [{ name: '入住城市', abb: '0', id: 'cityname', isMust: true, value: '<label class="input_area"><input type="text" name="cityname" id="cityname" value="" autocomplete="off"><input id="citynameInter" name="citynameInter" type="text" value=""  style="display: none;"  autocomplete="off" /></label>' },
                    ...
                    train: [
                            { name: '行程类型', abb: '0', isMust: false, id: 'trainType', value: '<div class="input_area"><label><input type="radio" name="train_type" checked="checked" class="f_type_ir">单程</label><label><input type="radio" name="train_type" class="f_type_ir">往返</label></div>', classs: 'search_radio_box' },
                            { name: '出发城市', abb: '1', isMust: true, id: 'trainDeptCity', value: '<label class="input_area"><input id="p_trainDcity" name="p_trainDcity"  value=""/><input type="hidden" value="" id="phid_tDcity" name="phid_tDcity" /></label>' },
                            { name: '到达城市', abb: '2', isMust: true, id: 'trainArriveCity', value: '<label class="input_area"><input id="p_trainAcity" name="p_trainAcity"  value=""/><input type="hidden" value="" id="phid_tAcity" name="phid_tDcity" /></label>' },
                            { name: '出发日期', abb: '3', isMust: true, id: 'trainDeptDate', value: '<label class="input_area"><input id="p_tDdate" type="text" value="" /></label>' },
                            { name: '返程日期', abb: '4', isMust: false, id: 'trainBackDate', value: '<label class="input_area"><input id="p_tAdate" type="text" value="" /></label>', classs: 'labgray' }
                        ]
                },
            ...
            maxHeight: function (doc, type) {
                var hotel = cQuery("#hotelOption", doc),
        ticket = cQuery("#ticketOption", doc),
        holiday = cQuery("#holidayOption", doc),
        group = cQuery("#groupOption", doc),
        train = cQuery('#trainOption', doc),
        searchBox = cQuery("#b2b_searchbox", doc);
                ...
                window.allHeight = {
                    ...
                    a: train.offset()['height']
                }

                var maxHeight = Math.max(window.allHeight.h, window.allHeight.t, window.allHeight.d, window.allHeight.g, window.allHeight.a);
                var b2bTitle = cQuery("#b2b_title", doc).offset()['height'];
                ...
                if (type != 'train') train.css('display', 'none');
        ...
        */
        ...
        cQuery.mod.load('page', '1.2');
        cQuery.mod.load('validate', '1.0');
        var ins;
        var mod = {
            doc: null,
            pop: null,
            init: function (doc, ifr) {
                if (!ins) {
                    ins = cQuery(document).regMod('validate', '1.0', {
                        callback: function () {
            ...
            inputTipInit: function (doc) {
                cQuery('input[type="text"]', doc).each(function (item, i) {
                    if (item.value() != '') {
            ...
            addressInit: function (doc) {
                //=酒店
                if (cQuery('#cityname', doc).length > 0) {
                    var a1 = cQuery('#cityname', doc).regMod('address', '1.0', {
                        name: 'cityname',
                        ...
                        row: 6,
                        relate: {
                            'id': cQuery('#cityId', doc)
                        }
                    });
                    // a1.method('validate');
                }
                if (cQuery('#citynameInter', doc).length > 0) {
                    var a2 = cQuery('#citynameInter', doc).regMod('address', '1.0', {
                        name: 'citynameInter',
                        ...
                        isAutoCorrect: true,
                        relate: {
                            'id': cQuery('#DistrictId', doc)
                        }
                    });
                }
                //=机票
                if (cQuery('#homecity_name', doc).length > 0) {
                    var ta1 = cQuery('#homecity_name', doc).regMod('address', '1.0', {
                        name: 'homecity_name',
                        //jsonpSource:'http://webresource.c-ctrip.com/code/cquery/resource/address/hotel/index/city_gb2312.js',
                        ...
                        isAutoCorrect: true,
                        relate: {
                            'code': cQuery('#homecity', doc)		//=todo 数据源 ID,code对应的值
                        }
                    });
                    // ta1.method('validate',false);
                }
                if (cQuery('#destcity1_name', doc).length > 0) {
                    var ta2 = cQuery('#destcity1_name', doc).regMod('address', '1.0', {
                        name: 'destcity1_name',
                        ...
                        isAutoCorrect: true,
                        relate: {
                            '2': cQuery('#destcity1', doc)	//=todo 数据源 ID,code对应的值
                        }
                    });
                    // ta2.method('validate',false);
                }
                if (cQuery('#homecity_nameOut', doc).length > 0) {
                    var ita1 = cQuery('#homecity_nameOut', doc).regMod('address', '1.0', {
                        ...
                        isAutoCorrect: true,
                        relate: {
                            'id': cQuery('#homecityOut', doc)		//=todo 数据源 ID,code对应的值
                        }
                    });
                    // ta1.method('validate',false);
                }
                if (cQuery('#destcity1_nameOut', doc).length > 0) {
                    var ita2 = cQuery('#destcity1_nameOut', doc).regMod('address', '1.0', {
                        ...
                        isAutoCorrect: true,
                        relate: {
                            '3': cQuery('#destcity1Out', doc)	//=todo 数据源 ID,code对应的值
                        }
                    });
                    // ta2.method('validate',false);
                }
                ...
                if (cQuery('#text1', doc).length > 0) {
                    var ha1 = cQuery('#text1', doc).regMod('address', '1.0', {
                        name: 'text1',
                        source: {
                            suggestion: {
                                '热门城市': [
                        ...
                        isAutoCorrect: true,
                        relate: {
                            'cityId': cQuery('#text1ID', doc)
                        }
                    });
                    // ha1.method('validate',false);
                }
                if (cQuery('#pkgdestCity', doc).length > 0) {
                    var ha2 = cQuery('#pkgdestCity', doc).regMod('address', '1.0', {
                        name: 'pkgdestCity',
                        ...
                        isAutoCorrect: false,
                        relate: {
                            'ids': cQuery('#pkgdestCityID', doc)
                        }
                    });
                }
                //=group
                if (cQuery("#groupCity", doc).length > 0) {
                    cQuery("#groupCity", doc).regMod('address', '1.0', {
                        name: 'groupCity',
                        source: {
                            suggestion: {
                        ...
                        isAutoCorrect: true,
                        relate: {
                            'cityId': cQuery('#grouphidCity', doc),
                            'pinyin': cQuery('#grouphidCityPinYin', doc)
                        }
                    })
                }
                //=train（hehe）
                if (cQuery('#p_trainDcity', doc).length > 0) {
                    cQuery('#p_trainDcity', doc).regMod('address', '1.0', {
                        name: 'p_trainDcity',
                        jsonpSource: 'http://webresource.ctrip.com/code/cquery/resource/address/train/station_gb2312.js',
                        isFocusNext: false,
                        isAutoCorrect: true,
                        charset: 'gb2312',
                        relate: {
                            'name_py': cQuery('#phid_tDcity', doc)
                        }
                    });
                }
                if (cQuery('#p_trainAcity', doc).length > 0) {
                    cQuery('#p_trainAcity', doc).regMod('address', '1.0', {
                        name: 'p_trainAcity',
                        jsonpSource: 'http://webresource.ctrip.com/code/cquery/resource/address/train/station_gb2312.js',
                        isFocusNext: false,
                        isAutoCorrect: true,
                        charset: 'gb2312',
                        relate: {
                            'name_py': cQuery('#phid_tAcity', doc)
                        }
                    });
                }
            },
            ...
            */
            calendarInit: function (doc, ifr) {
                ...
                ifm.marginHeight = "0";
                ifm.scrolling = "no";
                ifm.allowtransparency="yes";
                document.body.appendChild(ifm);
                ifm.style.cssText = "display:none;width:381px;height:215px;position:absolute;margin:0;padding:0;border:none;zIndex:9999;";
                if(document.all) ifm.style.width = "382px";
                ifm.contentWindow.document.open();
                ifm.contentWindow.document.write('\
                <!DOCTYPE html><html><head></head><body>\
                <div style="position:absolute;display:none;width:369px;*width:370px" id="calendars" class="calendar_wrap" >\
                <div class="calendar_month calendar" hidefocus="true" id="calendar0" >{$left}</div> \
                <div class="calendar_month calendar" hidefocus="true" id="calendar1" style="padding-left: 0;">{$right}</div>\
                </div></body></html>');
                ...
                if (cQuery('#starttime', doc).length > 0) {
                    cQuery('#starttime', doc).regMod('calendar', '3.0', {
                        options: {
                            showWeek: false,
                            inputWin: inputWin,
                        ...
                        listeners: {
                            onChange: function (input, value) {
                                if (input.id == "starttime") {
                                    inputWin.document.getElementById('deptime').setAttribute('startDate', value);
                                    inputWin.document.getElementById('deptime').focus();
                                    //cQuery('#inteDeptime',doc).data('minDate', value); 
                                }
                            }
                        }
                    });
                ...
                if (cQuery('#deptime', doc).length > 0) {
                    cQuery('#deptime', doc).regMod('calendar', '3.0', {
                        options: {
                            showWeek: false,
                            inputWin: inputWin,
                            calendarWin: calendarWin,
                ...
                //=ticket
                if (cQuery('#DDatePeriod1', doc).length > 0) {
                    cQuery('#DDatePeriod1', doc).regMod('calendar', '3.0', {
                        options: {
                            showWeek: false,
                            inputWin: inputWin,
                            calendarWin: calendarWin,
                        ...
                        listeners: {
                            onChange: function (input, value) {
                                if (input.id == "DDatePeriod1") {
                                    ...
                                    inputWin.document.getElementById('ADatePeriod1').focus();
                                }
                            }
                        }
                    });
                ...
                if (cQuery('#ADatePeriod1', doc).length > 0) {
                    cQuery('#ADatePeriod1', doc).regMod('calendar', '3.0', {
                        options: {
                            showWeek: false,
                            inputWin: inputWin,
                            calendarWin: calendarWin,
                ...
                //=train 
                if (cQuery('#p_tDdate', doc).length > 0) {
                    cQuery('#p_tDdate', doc).regMod('calendar', '3.0', {
                        options: {
                            showWeek: false,
                            inputWin: inputWin,
                            calendarWin: calendarWin,
                ...
                if (cQuery('#p_tAdate', doc).length > 0) {
                    cQuery('#p_tAdate', doc).regMod('calendar', '3.0', {
                        options: {
                            showWeek: false,
                            inputWin: inputWin,
                            calendarWin: calendarWin,
        ...
        var content = {
            ...
            dataType: 'hotel',
            sortInnerHTML: {
                hotel: '<a href="javascript:void(0);"><b></b><span>默认排序：携程推荐</span></a>\
	                <div class="c_sort_list" id="sort_select" style="width:142px;display:none;">\
		                <a href="javascript:void(0);" name="minPrice|desc">价格由高到低</a>\
		                <a href="javascript:void(0);" name="minPrice|asc">价格由低到高</a>\
		                <div class="value_nono"></div>\
                ...
                train: '<a href="javascript:void(0);"><b></b><span>默认排序：携程推荐</span></a>\
	                <div style="width:142px; display: none;" id="sort_select" class="c_sort_list">\
		                <a href="javascript:void(0);" name="startDate|asc">发车时间由早到晚</a>\
		                <a href="javascript:void(0);" name="startDate|desc">发车时间由晚到早</a>\
		                <div class="value_nono"></div>\
            ...
            init: function (doc) {
                ...
                this.loading = cQuery('#b2b_loading', content.doc);
                ...
                this.pagediv = cQuery('#b2b_page', content.doc);
                this.dataType = config.siteTypeValue;
                //this.searchBoxHeight =parseInt(cQuery("#b2b_searchbox", this.doc).offset()['height']);
                var searchTag = this.dataType + "Option"
                this.searchBoxHeight = parseInt(cQuery("#" + searchTag, this.doc).offset()['height']) + parseInt(cQuery("#b2b_title", this.doc).offset()['height'])
                this.bootBoxHeight = parseInt(cQuery("#b2b_foot", this.doc).offset()['height']);

                this.idParam = '&AllianceId=' + ctripUnion.AllianceId + '&sid=' + ctripUnion.SId + '&ouid=' + ctripUnion.OuId + '&app=' + ctripUnion.App;
                ...
                if (cQuery("#showLogo")[0]) { config.showLogo = cQuery("#showLogo")[0].checked }
                ...
                if (config.pageHeight < this.searchBoxHeight + this.bootBoxHeight) {
                    if (cQuery("#panelcurrentHeight")[0]) {
                        cQuery("#panelcurrentHeight")[0].value = this.searchBoxHeight + this.bootBoxHeight;
                        union.data.config.pageHeight = cQuery("#panelcurrentHeight")[0].value
                        config.pageHeight = cQuery("#panelcurrentHeight")[0].value;
                        //union.panelHeightChange(this.doc,union)
                    }
                }

                if (cQuery("#_main_box", this.doc)[0]) {

                    var mainboxHeight = content.getMainboxHeight(this.doc, config)

                    cQuery("#_main_box", this.doc)[0].style.height = mainboxHeight + 'px';
                }

                this.curRow = Math.floor((config.pageHeight - this.searchBoxHeight - this.bootBoxHeight - 45 - 36) / this.itemHeight);
                this.curCol = Math.floor((config.pageWidth - 40) / this.itemWidth);

                this.count = this.curRow * this.curCol > 0 ? this.curRow * this.curCol : 0;
                if (this.count == 0 || !config.showData || config.pageWidth < 270 || config.pageHeight - content.getSearchBoxHeight(content.doc, config) - 38 < content.minContentHeight) {

                    cQuery("#b2b_searchOption", this.doc)[0].style.height = content.getMainboxHeight(this.doc, config, "title") + 'px';
            ...
            getSearchBoxHeight: function (doc, config) {
                ...
                var config = ctripUnion.datas.config;
                var searchTitle = 0;
                if (config.siteMode == "all") { searchTitle = parseInt(cQuery("#b2b_title", doc).offset()['height']) }
                var searchTag = content.dataType + "Option"
                var searchBoxHeight = parseInt(cQuery("#" + searchTag, doc).offset()['height']) + searchTitle;
                ...
                if (cQuery("#showLogo", doc)[0]) { config.showLogo = cQuery("#showLogo", doc)[0].checked }
                if (config.showLogo == false) {
            ...
            getMainboxHeight: function (doc, config) {
                ...
                var config = ctripUnion.datas.config;
                var b2bTitlea = 0;
                if (config.siteMode == "all") { b2bTitlea = parseInt(cQuery("#b2b_title", doc).offset()['height']) }
                if (cQuery("#showLogo", doc)[0]) { config.showLogo = cQuery("#showLogo", doc)[0].checked }
                var mainboxHeight = parseInt(config.pageHeight) - parseInt(cQuery("#b2b_foot", doc).offset()['height']);
                var skin = config.skinNum ? config.skinNum : ctripUnion.datas.config.skinId;
                ...
                if (arguments.length > 2) { mainboxHeight = mainboxHeight - b2bTitlea; if (skin == 0) mainboxHeight -= 1 }
            ...
            accountPageInfo: function (action, isAccount) {
                ...
                var config = ctripUnion.datas.config;
                // var searchBoxHeight = parseInt(cQuery("#b2b_searchbox", this.doc).offset()['height']);
                var searchBoxWidth = parseInt(cQuery("#b2b_searchbox", this.doc).offset()['width']);
                var pageBoxHeight = 36; //cQuery("#b2b_page", this.doc).offset()['height'];
                var bootBoxHeight = parseInt(cQuery("#b2b_foot", this.doc).offset()['height']);

                var sortBoxHeight = (cQuery("#listSort", this.doc)[0].style.display == '' || cQuery("#listSort", this.doc)[0].style.display == 'block') ? 45 : 0;
                ...
                if (cQuery("#b2b_content", this.doc)[0]) {
                    cQuery("#b2b_content", this.doc)[0].style.height = this.contentHeight + 'px';
                }
                var curRow = Math.floor((this.contentHeight - sortBoxHeight) / this.itemHeight);
                if (cQuery("#showLogo")[0]) {
                    config.showLogo = cQuery("#showLogo")[0].checked;
                    config.showData = cQuery("#showDataList")[0].checked
                }

                if (config.showLogo == false) { curRow = Math.floor((this.contentHeight - sortBoxHeight) / this.itemHeight); }
                var curCol = Math.floor((config.pageWidth - 40) / this.itemWidth);
                var styleCol = Math.floor((config.pageWidth - 40) / this.itemWidth);
                ...
                if (config.pageWidth < 270) {
                    cQuery("#_maincontents", this.doc).addClass('min_box');
                    cQuery("#b2b_searchOption", this.doc)[0].style.height = content.getMainboxHeight(this.doc, config, "title") + "px";
                } else {
                    cQuery("#_maincontents", this.doc).removeClass('min_box');
                };
                ...
                var minHeight = serHeight + bootBoxHeight
                var searchTag = this.dataType + "Option"
                if (minHeight < 187) { minHeight = 187 }
                if (config.pageHeight < serHeight + 38) {
                    if (cQuery("#panelcurrentHeight")[0]) {
                        cQuery("#panelcurrentHeight")[0].value = minHeight
                        union.data.config.pageHeight = cQuery("#panelcurrentHeight")[0].value;
                        config.pageHeight = cQuery("#panelcurrentHeight")[0].value;
                        //  union.panelHeightChange(this.doc,union)
                    }
                    cQuery("#_main_box", this.doc)[0].style.height = content.getMainboxHeight(this.doc, config) + 'px';
                    if (!config.showLogo) {
                        cQuery("#b2b_searchOption", this.doc)[0].style.height = content.getMainboxHeight(this.doc, config, "title") + 38 + "px";
                    }
                }
                if (this.count == 0 && cQuery("#ctrip_union_product_20121221")[0]) {
                ...
                if (window["b2b_ctrip_v2"]) {
                    if (this.count == 0) {
                        bootBoxHeight = 38
                        if (config.pageHeight - content.getSearchBoxHeight(content.doc, config) - 38 <= content.minContentHeight) {
                            content.targ.style.visibility = 'hidden';
                            content.pagediv.css('visibility', 'hidden');
                            cQuery("#b2b_searchOption", this.doc)[0].style.height = content.getMainboxHeight(this.doc, config, "title") + "px";
                        } else {
                    ...
                    if (this.count > 0 && config.pageWidth > 269 && config.showData && config.pageHeight - content.getSearchBoxHeight(content.doc, config) - 38 >= content.minContentHeight) {
                        cQuery("#b2b_searchOption", this.doc)[0].style.height = "auto";
                    } else {
                        content.targ.style.visibility = 'hidden';
                        content.pagediv.css('visibility', 'hidden');
                        cQuery("#b2b_searchOption", this.doc)[0].style.height = content.getMainboxHeight(this.doc, config, "title") + "px";
            ...
            //绑定排序事件
            bindEvents: function () {
                var sort = cQuery('#sort_select', content.doc);
                var b2bcontent = cQuery("#b2b_content", content.doc);
                var selSort = cQuery('#selSort', content.doc).find("a");
                content.sortOption = content.doc.getElementById('sort_select');
                ...
                function () {
                    var obj = {};
                    var dat;
                    var val = cQuery(this)[0].name;
                    cQuery('#selSort', content.doc)[0].getElementsByTagName('span')[0].innerHTML = cQuery(this)[0].innerHTML
                    //将原始数据复制到一个全局变量
            ...
            reRenderData: function (num) {
                ...
                if (dat.length > 0) {
                    ...
                    var ct = cQuery.tmpl.render(tpl, obj);
                    ...
                    if (config.pageHeight - searchBoxHeight - bootBoxHeight < content.minContentHeight) {
                        ...
                        if (cQuery("#showDataList")[0]) { config.showData = cQuery("#showDataList")[0].checked }
                        if (config.showData == false) {
                            content.targ.style.visibility = 'hidden';
                            content.pagediv.css('visibility', 'hidden');
                        } else {
                            content.targ.style.visibility = '';
                            if (content.dataType == 'hotel' || content.dataType == 'train') content.sortTarg.style.display = '';
            ...
            getJSON: function (page, cache) {
                ...
                if (this.dataType == 'hotel') {
                    ctripUnion.App = '0104A00';
                } else if (this.dataType == 'ticket') {
                    ctripUnion.App = '0104B00';
                } else if (this.dataType == 'holiday') {
                    ctripUnion.App = '0104C00';
                } else if (this.dataType == 'group') {
                    ctripUnion.App = '0104D00';
                } else if (this.dataType == 'train') {
                    ctripUnion.App = '0104G00';
                }
                var _url = this.url + this.idParam.split('&app=')[0] + "&app=" + ctripUnion.App + '&pages=' + pages;
                if (!cache) {
                    _url += '&fct=' + (new Date()).getTime();
                }
                //解决 ajax 响应覆盖的问题 modify by hailiangli@Ctrip.com
                var requestID = cQuery("#hid_Ctrip_Union_ajaxRequest_dynamic_id")[0] || cQuery("#requestID", this.doc)[0];
                var isLoad = false;
                ...
                var bootBoxHeight = 38;
                var searchBoxHeight = content.getSearchBoxHeight(this.doc, config)
                if (cQuery("#ctrip_union_product_20121221")[0]) {
                    if (config.pageHeight - searchBoxHeight - bootBoxHeight < content.minContentHeight) {
                        return
                    }
                }
                ...
                var curRow = Math.floor((config.pageHeight - searchBoxHeight - parseInt(cQuery("#b2b_foot", this.doc).offset()['height']) - 45 - 36) / content.itemHeight);
                ...
                cQuery.jsonp(_url, {
                    ...
                    onload: function (obj) {
                        ...
                        if (config.pageHeight - searchBoxHeight - bootBoxHeight < content.minContentHeight && config.showData && !cQuery("#ctrip_union_product_20121221")[0]) {
                            if (config.siteMode == 'all') {
                                ...
                                if (/GetTrainList/gi.test(_url)) {
                                    hash['train'][pages] = obj.data;
                                }
                            }
                        }
                        if (isLoad) {
                            ...
                            if (config.pageHeight - searchBoxHeight - bootBoxHeight < content.minContentHeight) {
                                ...
                                if (config.showData) {
                                    ...
                                    content.loading.css('display', 'none');
                                    content.pagediv.css('visibility', 'visible');
                                    if (content.dataType == 'hotel' || content.dataType == 'train') {
                                        content.sortTarg.style.display = 'block';
                                        content.sortSel.innerHTML = content.dataType == 'hotel' ? content.sortInnerHTML.hotel : content.dataType == 'train' ? content.sortInnerHTML.train : '';
                                    }
            ...
            parseData: function (page) {
                ...
                //=取数据
                if (page > 1) {
                    if ((count * (cur - 1)) % 100 + count > 100 && content.endNum == false) {
                        cur = 1
                    } else {
                        cur = content.current
                    }
                ...
                if (data.length > 0) {
                    ...
                    if (content.endNum) { data = data.slice(start, data.length) } else {
                        data = data.slice(start, end);
                    }
                    if (content.oldDatas.length != 0 && start == 0 && page != 1 && content.endNum == false) {
                        data = content.oldDatas.concat(data)
                        data = data.slice(start, end);
                    }
            ...
            updateContent: function (dat) {
                //  content.doc.charset="utf-8";
                var tpl = this.getTpl();
                var ct = cQuery.tmpl.render(tpl, dat);
                this.targList.innerHTML = ct;
                //var ct2 = cQuery.tmpl.render(this.sorttmp,{opts:this.sortArr});
                content.bindEvents();
                ...
                if (config) {
                    switch (config.siteTypeValue) {
                        ...
                        case 'group': tpl = ctripUnion.grouptmp;
                            break;
                        case 'train': tpl = ctripUnion.traintmp;
                            break;
                    }
                }
                return tpl;
            ...
            pageChange: function (current) {
                ...
                if (current == content.max && c > content.objCount) {
                    page = Math.ceil(content.objCount / 100)
                    content.endNum = true
                } else {
                    content.endNum = false
                }
                if (hash[dtype][page] && 100 % content.count != 0) {
                    if (page > 1 && hash[dtype][page].length == 100) {
                        if (content.endNum) {
                            content.oldDatas.lenght = 0
                        } else {
                            content.oldDatas = hash[dtype][page - 1].slice((100 - 100 % content.count), 100)
                        }
                    } else {
                        if (content.endNum) {
                            content.oldDatas.lenght = 0
                        } else {
                            content.oldDatas = hash[dtype][1].slice((100 - 100 % content.count), 100)
                        }
            ...
            pageInit: function (current, max, callback) {
                ...
                if (this.page) {
                    var targ = cQuery('#b2b_page', content.doc);
        ...
        var groupValidate = function (doc, obj, isrun) {
            //var _url = content.ctripUnion.hostAddress + 'union/Ajax/AjaxServiceForSearch.ashx?action=GetGroupList';
            var _url = 'http://u.ctrip.com/union/Ajax/AjaxServiceForSearch.ashx?action=GetGroupList';
            var paramobj = {
                incity: null,
                incityid: null,
                incityPY: null,
            ...
            function CheckPrice() {
                var s = false, e = false;
                if (cQuery('#groupdayprice', doc).length > 0) {
                    if (cQuery('#groupdayprice', doc)[0].value.length > 0) {
                        paramobj.sprice = cQuery('#groupdayprice', doc)[0].value;
                        s = true;
                    } else {
                ...
                if (cQuery('#groupdayprice2', doc).length > 0) {
                    if (cQuery('#groupdayprice2', doc)[0].value.length > 0) {
                        paramobj.eprice = cQuery('#groupdayprice2', doc)[0].value;
                        e = true;
                    } else {
                ...
                if (s && e) {
                    if (parseInt(cQuery('#groupdayprice', doc)[0].value) > parseInt(cQuery('#groupdayprice2', doc)[0].value)) {
                        mod.warn(cQuery('#groupdayprice', doc)[0], '起始价格不能大于截止价格!');
                        return false;
                    }
                }
                return true;
            ...
            function CheckCityName() {
                if (cQuery("#groupCity", doc).length > 0) {
                    paramobj.incity = escape(cQuery("#groupCity", doc)[0].value);
                };
                if (cQuery("#grouphidCity", doc).length > 0) {
                    paramobj.incityid = cQuery("#grouphidCity", doc)[0].value;
                };
                if (cQuery('#grouphidCityPinYin', doc).length > 0) {
                    paramobj.incityPY = cQuery('#grouphidCityPinYin', doc)[0].value;
                }
            };
            function setUrl() {
                var param = '';
                ...
                CheckCityName();
                if (cQuery('#select_groupsortType', doc).length > 0) {
                    paramobj.orderby = cQuery('#select_groupsortType', doc)[0].value;
                };
            ...
            cQuery("#groupSearchBt", doc).bind('click', function () {
                if (cQuery("#showDataList")[0]) {
            ...
            if (cQuery('#groupdayprice', doc).length > 0) {
                cQuery('#groupdayprice', doc).bind('keyup', function () {
                    verifyKeyPress(this, "0", "999999");
                });
            };
            if (cQuery('#groupdayprice2', doc).length > 0) {
                cQuery('#groupdayprice2', doc).bind('keyup', function () {
                    verifyKeyPress(this, "0", "999999");
                });
            }
        };
        var trainValidate = function (doc, obj, isrun) {
            var _url = 'http://u.ctrip.com/union/Ajax/AjaxServiceForSearch.ashx?action=GetTrainList';
            var params = {
                deptCity: null,
                deptCityId: null,
                deptDate: null,
            ...
            function verify() {
                mod.warn();
                var typebtns = cQuery('.f_type_ir[name=train_type]', doc);
                var type = typebtns.length > 0 ? typebtns[0].checked ? 's' : 'd' : 's';
                var _findError = false, res = true;
                var _dcity = cQuery('#p_trainDcity', doc);
                var _acity = cQuery('#p_trainAcity', doc);
                var _ddate = cQuery('#p_tDdate', doc);
                var _adate = cQuery('#p_tAdate', doc);

                if (_dcity.length > 0 && _dcity.value().length == 0) {
                    mod.warn(_dcity[0], '请选择出发城市');
                    _findError = true; res = false;
                }
                if (_acity.length > 0 && _acity.value().length == 0 && !_findError) {
                    mod.warn(_acity[0], '请选择到达城市');
                    _findError = true; res = false;
                }
                if (_ddate.length > 0 && _ddate.value().length == 0 && !_findError) {
                    mod.warn(_ddate[0], '请选择出发日期');
                    _findError = true; res = false;
                } else if (!_ddate.value().toDate() && !_findError) {
                    mod.warn(_ddate[0], '出发日期不是有效的日期格式');
                    _findError = true; res = false;
                }
                if (type === 'd' && _adate.length > 0 && _adate.value().length == 0 && !_findError) {
                    mod.warn(_adate[0], '请选择返程日期');
                    _findError = true; res = false;
                } else if (type === 'd' && _adate.length > 0 && !_adate.value().toDate() && !_findError) {
                    mod.warn(_adate[0], '返程日期不是有效的日期格式');
                    _findError = true; res = false;
                }
                if (_dcity.length > 0 && _acity.length > 0 && _dcity.value().trim() == _acity.value().trim() && !_findError) {
                    mod.warn(_acity[0], '出发与到达城市重复');
                    _findError = true; res = false;
                }
                if (type === 'd' && _ddate.length > 0 && _adate.length > 0 && !_findError) {
                    var _dDateTime = new Date(_ddate.value().replace(/\-/ig, '/')).getTime();
                    var _aDateTime = new Date(_adate.value().replace(/\-/ig, '/')).getTime();
                    if (_aDateTime < _dDateTime) {
                        mod.warn(_adate[0], '返程日期不能早于出发日期');
                        _findError = true; res = false;
                    }
                }
            ...
            function buildPars() {
                params.deptCity = escape(cQuery('#p_trainDcity', doc).value());
                params.deptCityId = cQuery('#phid_tDcity', doc).value();
                params.arriCity = escape(cQuery('#p_trainAcity', doc).value());
                params.arriCityId = cQuery('#phid_tAcity', doc).value();
                params.deptDate = cQuery('#p_tDdate', doc).value();
                params.arriDate = cQuery('#p_tAdate', doc).value();
                params.trainType = cQuery('.f_type_ir[name=train_type]', doc).length > 0 ? cQuery('.f_type_ir[name=train_type]', doc)[0].checked ? 's' : 'd' : 's';
            };
            function setUrl() {
                var uri_pars = '';
                buildPars();
                ...
                for (var key in params) {
                    uri_pars += '&' + key + '=' + params[key];
                };
                return _url + uri_pars;
            };

            if (!cQuery("#ctrip_union_product_20121221")[0] || cQuery("#ctrip_union_product_20121221")[0]) {
                if (verify() && isrun == void 0) {
                    content.url = setUrl();
                    content.getJSON();
                };
            };

            cQuery('#trainSearchBt', doc).bind('click', function () {
                var showDataC;
                if (cQuery("#showDataList")[0]) {
                    showDataC = union.data.config.showData;
                } else {
                    showDataC = content.ctripUnion.datas.config.showData;
                }
                var config = content.ctripUnion.datas.config
                var bootBoxHeight = 38;
                var searchBoxHeight = content.getSearchBoxHeight(doc, config)
                if ((config.pageHeight - searchBoxHeight - bootBoxHeight < content.minContentHeight
                && content.ctripUnion.datas.config.showData) || (config.pageWidth < 270)) {
                    showDataC = false;
                };
                if (verify()) {
                    if (showDataC) {
                        ...
                        buildPars();
                        var uri = ["http://u.ctrip.com/union/CtripRedirect.aspx?AllianceID=" + content.ctripUnion.AllianceId];
                        uri.push("SID=" + content.ctripUnion.SId);
                        uri.push("Ouid=" + content.ctripUnion.OuId);
                        if (params.trainType == 's') {
                            uri.push("TypeID=90");
                            uri.push("StartDate=" + params.deptDate.trim());
                            uri.push("StartCity=" + params.deptCityId.trim());
                            uri.push("DestCity=" + params.arriCityId.trim());
                            uri.push("Number=");
                        } else {
                            uri.push("TypeID=92");
                            uri.push("StartDate=" + params.deptDate.trim());
                            uri.push("StartCity=" + params.deptCityId.trim());
                            uri.push("DestCity=" + params.arriCityId.trim());
                            uri.push("ReturnDate=" + params.arriDate.trim());
                            uri.push("Number=");
                            uri.push("StartCityCn=" + escape(params.deptCity));
                            uri.push("DestCityCn=" + escape(params.arriCity));
                        };
                        window.open(uri.join('&'));
                    };
                };
            });
        };
        var hotelValidate = function (doc, obj, isrun) {
            var _url = 'http://u.ctrip.com/union/Ajax/AjaxServiceForSearch.ashx?action=GetHotelList';
            //var _url = 'http://u.testu.sh.ctriptravel.com/union/Ajax/AjaxServiceForSearch.ashx?action=GetHotelList';
            var paramobj = {
                incity: '',
                indate: '',
                outdate: '',
            ...
            function isTWCity() { //判断如果是台湾的城市，跳转到海外酒店
                ...
                for (var i = 0, l = TW.length; i < l; i++) {
                    // if(cv == cQuerys2t(TW[i])){return true;}
            ...
            function checkTime(id) {
                ...
                flag[id] = v;
                if (id == 'starttime') {
                    /*if(new Date()> flag['starttime'])  {
                        targ.className = "f_error";
                        mod.warn(targ, "入住时间不能早于当前时间");
                        return false;
                    }   */
            ...
            cQuery('.f_type_ir[name="hotelType"]', doc).each(function (item, i) {
            ...
            cQuery('#hotelsubmit', doc).bind('click', function () {
                param = '';
                mod.warn();
                for (var i in obj) {
                    var o = obj[i], v = '';
        ...
        var ticketValidate = function (doc, obj, isrun) {
            var isSetUrl = true

            if (cQuery("#ticketSearchBt", doc).length > 0) {
                cQuery("#ticketSearchBt", doc).bind('click', function (e) {
                    isSetUrl = true;
                    isWinOpen = true;
                    tovalidate(doc);
                });
            ...
            var flightSwitch = cQuery("*[name='flightSwitch']", doc);
            if (flightSwitch) {
                flightSwitch.bind("click", function (e) {
                    ...
                    if (tag.value == 0) {
                        cQuery("#homecity_name", doc)[0].style.display = "";
                        cQuery("#destcity1_name", doc)[0].style.display = "";
                        cQuery("#homecity_nameOut", doc)[0].style.display = "none";
                        cQuery("#destcity1_nameOut", doc)[0].style.display = "none";
                        if (cQuery("#showDataList")[0]) {
                            var showDataC = union.data.config.showData;
                        } else {
                            var showDataC = content.ctripUnion.datas.config.showData;
                        ...
                        cQuery("#homecity_name", doc)[0].style.display = "none";
                        cQuery("#destcity1_name", doc)[0].style.display = "none";
                        cQuery("#homecity_nameOut", doc)[0].style.display = "";
                        cQuery("#destcity1_nameOut", doc)[0].style.display = "";
                        if (cQuery("#showDataList")[0]) {
                            var showDataC = union.data.config.showData;
                        } else {
                            var showDataC = content.ctripUnion.datas.config.showData;
            ...
            var tovalidate = function (doc) {
                ...
                var flightBackFlag = cQuery("#flightBackFlag")[0]; //div
                ...
                var destcity1Code = cQuery("#destcity1", doc);
                var flightTag = true; //是否是国内机票
                var flightSwitch = cQuery("*[name='flightSwitch']", doc);  //需要改变
                ...
                function setFlightSource() {
                    if (flightTag) { //国内机票
                        ...
                        cQuery(homecity).module.address.source = "fltDomestic";
                        cQuery(destcity1).module.address.source = "fltDomestic";
                ...
                var beginvalidate = function () {
                    var isIntFlt = function () {
                        if (cQuery("*[name='flightSwitch']", doc)[0].checked == true)
                            return false;
                        else
                            return true;
                    }
                    ...
                    var urls = null;
                    if (!isIntFlt()) {
                        datas.push("&dcity=" + cQuery("#homecity", doc)[0].value);
                        datas.push("&acity=" + cQuery("#destcity1", doc)[0].value);
                        urls = _url + "GetAirPlanList";
                    } else {
                        datas.push("&dcity=" + cQuery("#homecityOut", doc)[0].value);
                        datas.push("&acity=" + cQuery("#destcity1Out", doc)[0].value);
                    ...
                    if (!isSetUrl) {
                        ...
                        if (showDataC) {
                            ...
                            if (!isIntFlt()) {
                                ...
                                airplanurl += "&dcityname1=" + escape(cQuery("#homecity_name", doc).value());
                                airplanurl += "&acityname1=" + escape(cQuery("#destcity1_name", doc).value());
                                airplanurl += "&dcity1=" + cQuery("#homecity", doc)[0].value;
                                airplanurl += "&acity1=" + cQuery("#destcity1", doc)[0].value;
                                ...
                                //舱位级别
                                if (form["siteLevel"] && form["siteLevel"].value) {
                                    airplanurl += "&classtype=" + (form["siteLevel"].value === "0" ? "" : form["siteLevel"].value);
                                } else {
                                ...
                                airplanurl += "&dcity=" + cQuery("#homecityOut", doc)[0].value;
                                airplanurl += "&acity=" + cQuery("#destcity1Out", doc)[0].value;
                                ...
                                if (sDate && eDate) {
                                    ...
                                    airplanurl += "&relddate=" + content.ctripUnion.getDateDiff(nowDate, DDatePeriod1.value, "DAY");
                                    airplanurl += "&relrdate=" + content.ctripUnion.getDateDiff(nowDate, ADatePeriod1.value, "DAY");
                                } else if (!sDate && !eDate) {
                                    airplanurl += "&flightway=single";
            ...
            if (!cQuery("#ctrip_union_product_20121221")[0] || cQuery("#ctrip_union_product_20121221")[0]) {
                isSetUrl = false;
                if (isrun == void 0) tovalidate(doc);
            }
        }

        var holidayValidate = function (doc, obj, isrun) {
            ...
            cQuery('#holidaysubmit', doc).bind('click', function () {
                //text1 pkgdestCity
                if (cQuery("#showDataList")[0]) {
                    var showDataC = union.data.config.showData;
                } else {
                ...
                if (showDataC) {
                    ...
                    if (param.dcity) holidayurl += "&FilterPara=" + param.dcity;
        ...
        if (window['b2b_ctrip_v2']) {
            ...
            content.callback = function (doc) {
                ...
                if (window["ctripHcallback"]) {
                    ...
                    b2bContent = cQuery("#b2b_content", doc)[0]
        ...
        */
        ...
        function init(ifr, ctripUnion) {
            ...
            if (config.siteMode == 'simple') {
                ...
                if (config.siteTypeValue == 'train') {
                    trainValidate(doc, ctripUnion.defineData.searchOption.train);
                };
            } else {
                if (config.siteTypeValue == 'hotel') {
                    ticketValidate(doc, ctripUnion.defineData.searchOption.ticket, false);
                    holidayValidate(doc, ctripUnion.defineData.searchOption.holiday, false);
                    groupValidate(doc, ctripUnion.defineData.searchOption.group, false);
                    trainValidate(doc, ctripUnion.defineData.searchOption.train, false);
                    hotelValidate(doc, ctripUnion.defineData.searchOption.hotel);
                ...
                if (config.siteTypeValue == 'ticket') {
                    ...
                    holidayValidate(doc, ctripUnion.defineData.searchOption.holiday, false);
                    groupValidate(doc, ctripUnion.defineData.searchOption.group, false);
                    hotelValidate(doc, ctripUnion.defineData.searchOption.hotel, false);
                    ticketValidate(doc, ctripUnion.defineData.searchOption.ticket);
                }
                if (config.siteTypeValue == 'holiday') {
                    trainValidate(doc, ctripUnion.defineData.searchOption.train, false);
        ...
        function triggerClick(dtype, doc, obj) {
            ...
            var bootBoxHeight = parseInt(cQuery("#b2b_foot", doc).offset()['height']);
...
/*if(!window['cQuery']){
    ...
    cQuery.config('namespace', 'cQuery');
    cQuery.ready(init);
    }
    s.src = 'http://webresource.ctrip.com/code/cquery/cQuery_110421.js';
...
if(document.getElementById("ctrip_union_code_preview")){
    window.onload=unionDomLoad
}else{
    if (window.attachEvent){
       window.attachEvent('onload',unionDomLoad)
    }
    else{
    window.addEventListener('load', unionDomLoad,false);
    }
}
function unionDomLoad() {
        if (!window['cQuery']) {
